...
Shared-z+GR00T is already a one-pass latent version of this idea: the supervised128D z enters DiT AdaLN and the winning configuration masks ordinary encoder memory completely. That result supports a rollout-visible bottleneck. It does not show that feeding decoded auxiliary outputs back into the encoder is redundant. A head prediction is a deterministic readout from h or z; merely making it decodable does not force the action head to use it. The main missing mechanism test is tied two-pass bidirectional trace re-encoding without z: pass1 predicts five ordered coordinates from full encoder memory, only rounded/Fourier-embedded coordinates enter pass2, and GR00T consumes all pass2 states. This mirrors the causal winner's generate→re-encode→act mechanism while allowing the bidirectional second pass to refine all modalities around the proposed plan. Pass2 z plus contextual trace states is the subsequent performance combination. Direct coordinate tokens into GR00T are the required one-pass ablation; mid-layer loopback is a later efficiency approximation.
