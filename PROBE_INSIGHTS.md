...
Shared-z+GR00T is already a one-pass latent version of this idea: the supervised128D z enters DiT AdaLN and the winning configuration masks ordinary encoder memory completely. That result supports a rollout-visible bottleneck. It does not show that feeding the decoded auxiliary outputs back into the encoder is redundant. A head prediction is a deterministic readout from h or z; merely making it decodable does not force the action head to use it. The next identifiable test is six ordered continuous trace tokens from full encoder memory, exposed alongside z while ordinary memory remains masked, followed by a mid-layer bidirectional loopback only if correct-versus-shuffled trace tokens causally affect actions.
