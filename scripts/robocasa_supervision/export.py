    ...
    with ExitStack() as stack:
        ...
        for row in expected:
            ...
            for t in range(meta['n_frames']):
                ...
                z.write(json.dumps({'trajectory_name':name,'frame_index':t,'subtask_id':sid,'targets':targets,'valid':masks,'future_frame_index':int(data['future_frame_index'][t]),'future_frame_valid':bool(data['future_frame_valid'][t]),'dense_source':str(root/'targets.npz')},separators=(',',':'))+'\n');counts['z_rows']+=1
