...
# examples/simBenchmarks/LIBERO-plus/eval_files/parallel_eval/auto_eval_libero_plus.sh.
#
...
for ((u=0; u<${#MY_UNITS[@]}; u++)); do
    ...
    (
        ...
        CUDA_VISIBLE_DEVICES="${gpu_id}" MUJOCO_EGL_DEVICE_ID="${gpu_id}" "${VLA_ARENA_python}" \
            examples/simBenchmarks/VLA-Arena/eval_files/eval_vla_arena.py \
...
if [ "${SKIP_AGGREGATE}" != "true" ]; then
    python examples/simBenchmarks/VLA-Arena/eval_files/parallel_eval/aggregate_vla_arena_results.py --root_path "${output_dir}"
fi
