...
### Apptainer image (preferred on this cluster)

An Apptainer image bundles upstream `robocasa/main`, `robosuite/master`, and all
kitchen assets, so the simulator never has to be installed per-node:
...
`/opt/starvla-build-info/` inside the SIF. The simulator's many asset files live
in the single SquashFS image inode. The StarVLA model server still runs in the
host `starVLA` conda environment.
...
tree stays in the read-only SquashFS image; only those small copy-on-write
changes occupy node memory for the lifetime of the evaluation process.

Select it explicitly with `ROBOCASA365_RUNTIME=apptainer`, or point
`ROBOCASA365_SIF` at another image. The conda installation below remains the
fallback.
