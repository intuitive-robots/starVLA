...
Schedule: Day1 generation, loader/augmentation integration and20-update GR00T smokes1863647/48 completed. A flaw was then found in z-only conditioning, so full RoboCasa jobs1863688/90/92, resume jobs1863689/91/93 and evals1863697–702 were cancelled. Treat the smokes as data-path diagnostics only and do not resume their run IDs. Day2 audits and fixes the conditioning path, proves the intended action-token information flow in a forward/backward test, and repeats the20-update trainer-eval gate before any full launch. The pre-existing PI-v4 RoboCasa run1858730 continues. Keep the Sep21 scientific gate and Sep24 headline freeze.
