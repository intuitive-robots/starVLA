...
Schedule: Day1 generation and native loader/augmentation integration completed. The verified GR00T+z path and three matched configs are now committed in the RoboCasa worktree: no-z, z-only with all GR00T readout memory suppressed, and z+retained-memory with memory kept on85% of examples. Every arm preserves direct32-D state, uses the exact winning LIBERO GR00T head, global batch64 and the joint-augmentation switch. Smokes1863647/48 are submitted as two runs per4-GPU node for20 updates, trainer evaluation at10/20 and a step20 checkpoint. Day2 begins by auditing finite losses, all six z-target coverages, intended gradients, memory keep rates and checkpoint completeness; only passing arms advance immediately to full seeds4/42. Chain the declared18-task evaluation after each full pair. Keep the Sep21 scientific gate and Sep24 headline freeze.
